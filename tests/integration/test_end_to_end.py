...
from aether_frame.contracts import (
    ...
    UniversalMessage, AgentConfig, ExecutionConfig, UniversalTool,
    FrameworkType, TaskStatus, ExecutionMode, TaskComplexity
)
...
class TestEndToEndFlow:
    ...
    async def test_simple_chat_task_processing(self, ai_assistant):
        ...
        assert result.status in [TaskStatus.SUCCESS, TaskStatus.ERROR]  # Should be one of these
    ...
    async def test_task_routing_logic(self):
        ...
        assert strategy.framework_type == FrameworkType.ADK  # Should prefer ADK for chat
        assert strategy.agent_type == "conversational_agent"
        ...
        assert complex_strategy.framework_type in [FrameworkType.ADK, FrameworkType.LANGGRAPH]
        assert complex_strategy.agent_type == "research_agent"
    ...
    def test_data_contracts_integration(self):
        ...
        from aether_frame.contracts import UniversalTool, KnowledgeSource
...
if __name__ == "__main__":
    # Run specific test
    pytest.main([__file__ + "::TestEndToEndFlow::test_simple_chat_task_processing", "-v"])