...
from aether_frame.contracts import (
    TaskRequest, UserContext, SessionContext, UniversalMessage, 
    FrameworkType, ExecutionConfig, ExecutionMode
)
...
async def test_real_end_to_end_flow():
    ...
    try:
        ...
        flow_success = (
            result.task_id == task_request.task_id and
            result.status is not None and
            (result.status.name in ['SUCCESS'] or result.error_message is not None)
        )
