...
class TestAdkLiveCommunicator:
    ...
    async def test_send_user_response(
        self,
        mock_part,
        mock_content,
        adk_communicator,
        ...
        mock_live_request_queue.send_content.assert_called_once_with(
    ...
    @patch("google.genai.types.Content")
    @patch("google.genai.types.Part")
    async def test_send_cancellation(
        self, mock_part, mock_content, adk_communicator, mock_live_request_queue
    ):
    ...
    @patch("google.genai.types.Content")
    @patch("google.genai.types.Part")
    async def test_send_cancellation_default_reason(
        self, mock_part, mock_content, adk_communicator, mock_live_request_queue
    ):
    ...
    @patch("google.genai.types.Content")
    @patch("google.genai.types.Part")
    async def test_send_user_message(
        self, mock_part, mock_content, adk_communicator, mock_live_request_queue
    ):
    ...
    async def test_operations_after_close_raise_error(
        self, mock_part, mock_content, adk_communicator, sample_interaction_response
    ):
