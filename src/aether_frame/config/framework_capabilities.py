...
class FrameworkCapabilityConfig:
    ...
    execution_modes: List[str] = field(default_factory=list)  # workflow, plan-execution, react, etc.
...
def get_framework_capability_config(framework_type: FrameworkType) -> FrameworkCapabilityConfig:
    """Get capability configuration for a framework."""
    if framework_type not in FRAMEWORK_CAPABILITIES:
        raise ValueError(f"Framework {framework_type} capability configuration not found")
    
    return FRAMEWORK_CAPABILITIES[framework_type]


def framework_supports_capability(framework_type: FrameworkType, capability: str) -> bool:
